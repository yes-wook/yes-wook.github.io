...
작년 성능 향상 자체가 틀렸다는 뜻은 아니었습니다. 파일 스캔이 줄고 실행시간이 개선된 건 그때도 실제로 있었던 결과였습니다. 다만 이번에 다시 보니 Bloom Filter가 파일을 걸러주는 효과는 크지 않았고, 오히려 Photon 경로를 막고 있다는 게 새롭게 드러났습니다. 이 억제 효과가 작년에도 있었는지는 이번 재검증만으로는 알 수 없었지만, 지금 조건에서는 Bloom Filter가 얻는 것보다 잃는 게 크다는 것만은 분명했습니다. 이 시점부터 Bloom Filter보다는 Z-ORDER를 다시 검토해야겠다고 판단했습니다.
