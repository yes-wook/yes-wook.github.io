...
삭제 요청 →
  _delta_log JSON 업데이트
    ...
    └── add 액션 (DV 메타데이터 참조)
  ↓
  Deletion Vector 파일(.bin) 생성
    └── 삭제된 row 위치를 비트맵/리스트로 저장
```
<p>쿼리 엔진 동작 순서:</p><ol><li>_delta_log에서 최신 스냅샷(유효한 파일 목록) 확인</li><li>유효한 Parquet 파일 + 연결된 DV 메타데이터 로딩</li><li>DV 메타데이터에 지정된 .bin 파일 참조</li><li>삭제된 row를 제외하고 결과 반환</li></ol><h4>📂 실제 디렉토리 구조 예시 (S3)</h4><p>파티션이 설정된 델타 테이블기준으로 아래와 같은 디렉토리 구성됨</p>
```text
tables/
└── /delta-table/                              # Delta Table Root
    ├── _delta_log/                            # 트랜잭션 로그
    │   ├── 00000000000000000010.json
    │   ├── 00000000000000000011.json
    │   └── 00000000000000000012.checkpoint.parquet
    │
    ├── 01/                                    # 파티션 디렉토리
    │   ├── part-00000-xxxx.snappy.parquet
    │   └── part-00001-xxxx.snappy.parquet
    │
    ├── deletion_vector_0014c02e-....bin       # DV 파일 (삭제된 row 기록)
    └── deletion_vector_0a7d33ab-....bin
```
<p><strong>_delta_log/</strong></p><ul><li>JSON + Checkpoint Parquet</li><li>어떤 파일이 추가/삭제되었는지, 그리고 DV 메타데이터를 기록</li></ul><p><strong>파티션 디렉토리 (예: 01/)</strong></p><ul><li>실제 데이터 파일(Parquet)이 저장되는 위치</li></ul><p><strong>deletion_vector_xxxx.bin</strong></p><ul><li>삭제된 row들의 위치(비트맵/리스트)를 저장하는 별도 바이너리 파일</li><li>_delta_log의 add 액션에서 이 파일을 참조 (pathOrInlineDv 필드)</li></ul><h4>📑 DV 메타데이터 예시</h4><p><strong>remove 액션</strong></p><p>_delta_log의 remove액션으로 기존 로그를 무효화합니다.</p>
```json
{
  "remove": {
